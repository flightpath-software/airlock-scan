...
The axes run as **parallel sub-agents** so they don't pollute each other's context, then this
skill aggregates their findings.

This repo primarily uses GitHub Issues, but a spec/plan document under `docs/` may also be the source.

## Process

### 1. Pin the fixed point

Whatever the user said is the fixed point — a commit SHA, branch name, tag, `develop`, `HEAD~5`, etc.
If they didn't specify one, ask for it. The integration branch is **`develop`**, so a common fixed
point is the merge-base with `develop`.
...
1. Issue references in the commit messages (`#123`, `Closes #45`) — fetch the issue via the GitHub
   MCP tools (`mcp__github__issue_read` / `search_issues`; load via ToolSearch — query `github issue`).
2. A path the user passed as an argument.
3. A plan/spec file under `docs/` (e.g. `docs/project-plan.md`, `docs/project-plan-public.md`,
   `docs/future-work.md`) matching the branch name or feature.
...
The repo's documented standards, in priority order:

1. [`docs/CODING_STANDARDS.md`](../../../docs/CODING_STANDARDS.md) — the canonical conventions.
2. [`CLAUDE.md`](../../../CLAUDE.md) — house rules (the tier doctrine, local-only reports, corpus
   safety).
3. [`CONTRIBUTING.md`](../../../CONTRIBUTING.md) — commits, testing, and the branch model.
4. The accepted [`docs/adr/`](../../../docs/adr/) decisions — a diff must not violate an accepted ADR.
...
- **Always a judgement call.** Each smell is a labelled heuristic ("possible Feature Envy"), never a
  hard violation — and, like any standard here, skip anything tooling already enforces (see the
  "Enforced by tooling" list in `docs/CODING_STANDARDS.md`).
...
Send a single message with **two** `Agent` tool calls (use the `general-purpose` subagent for each)
so they run concurrently and don't share context.
...
- The brief: "Report — per file/hunk where relevant — (a) every place the diff violates a documented
  ...
  quote the hunk. Distinguish hard violations (a documented standard or an accepted ADR breached)
...
Present the reports under `## Standards` and `## Spec` headings, verbatim or lightly cleaned. Do
**not** merge or rerank findings — the axes are deliberately separate (see _Why the axes are
separate_).

End with a one-line summary: total findings per axis and the worst issue _within each axis_. Don't
pick a single winner across axes — that's the reranking the separation exists to prevent.
...
Reporting them separately stops one axis from masking another. Both are judgement calls — keep them
visibly distinct so a "looks fine" review can't paper over one with the other.
