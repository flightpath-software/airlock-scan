...
`airlock-scan` (`airlock`) is a **shell-first, `uv`-native** toolkit that vets an
...
- `bin/airlock` — gum launcher (main entry point); `scripts/` — shell UX.
- `scanners/` — one adapter per external tool; `config/` — scanner registry + Semgrep rules.
- `src/airlock_scan/` — Python helper: `config`, `store`, `database`, `canary`,
...
attack surface — the same hazard airlock exists to defend against (see
...
- Run the Tier-2 reviewer: `airlock-helper quarantine <dir>` (needs `OPENAI_API_KEY`,
  or `--fake` for offline). It reviews at most `AIRLOCK_LLM_MAX_FILES` files (default 5).
...
airlock changelog preview    # preview the unreleased section
# or directly:
...
`pyproject.toml` + `src/airlock_scan/__init__.py` + `uv.lock`, commits, and tags
`vX.Y.Z`. towncrier owns the changelog; commitizen owns versioning/tagging
(`update_changelog_on_bump = false`, `version_provider = "uv"`).

```bash
airlock release            # interactive (confirms each step)
airlock release --yes      # non-interactive (CI)
...
Preview without changing anything: `airlock changelog preview` or
...
- Reports stay **local to the user** (`~/airlock/`), never written into the scanned
